...
class CalculatorDisplay(Gtk.Box):
    ...
    def __init__(self, **kwargs):
        ...
        # Header display entry (editable)
        self.display_entry = Gtk.Entry()
        self.display_entry.set_alignment(1.0) # Right align
    ...
    def insert_at_cursor(self, text):
        ...
        # Direct update (this will trigger `changed` but we can filter or just emit user-edited)
        # Actually, if we update via set_text, 'changed' fires.
        # We want to emit 'user-edited' so the logic updates.
